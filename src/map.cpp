...
// 11.12.13 упрощено Прулем! (prool)
...
SPECIAL(guild_poly);
SPECIAL(torc);
//SPECIAL(Noob::outfit); // prool
extern int has_boat(CHAR_DATA *ch);

int map_check_option(int parameter) // prool
{
return 1;
}
...
void draw_mobs(const CHAR_DATA *ch, int room_rnum, int next_y, int next_x)
{
	...
	else
	{
		...
		for (CHAR_DATA *tch = world[room_rnum]->people; tch; tch = tch->next_in_room)
		{
			...
			if (IS_NPC(tch) && !map_check_option(MAP_MODE_MOBS))
			{
				continue;
			}
			if (!IS_NPC(tch) && !map_check_option(MAP_MODE_PLAYERS))
			{
				continue;
			}
			if (HERE(tch)
	...
	else
	{
		...
		for (OBJ_DATA *obj = world[room_rnum]->contents; obj; obj = obj->next_content)
		{
			if (IS_CORPSE(obj) && GET_OBJ_VAL(obj, 2) >= 0
				&& !map_check_option(MAP_MODE_MOBS_CORPSES))
			{
				continue;
			}
			if (IS_CORPSE(obj) && GET_OBJ_VAL(obj, 2) < 0
				&& !map_check_option(MAP_MODE_PLAYER_CORPSES))
			{
				continue;
			}
			if ((GET_OBJ_TYPE(obj) == ITEM_INGRADIENT || GET_OBJ_TYPE(obj) == ITEM_MING)
				&& !map_check_option(MAP_MODE_INGREDIENTS))
			{
				continue;
			}
			if (!IS_CORPSE(obj)
				...
				&& !map_check_option(MAP_MODE_OTHER_OBJECTS))
			{
				continue;
			}
			if (1/*CAN_SEE_OBJ(ch, obj)*/) // prool
			{
				++cnt;
			}
		}
...
{
	bool all = map_check_option(MAP_MODE_MOB_SPEC_ALL) ? true : false;
...
{
	if (map_check_option(MAP_MODE_1_DEPTH)
		&& !map_check_option(MAP_MODE_2_DEPTH)
		&& cur_depth > 1)
	{
		return false;
	}

	if (map_check_option(MAP_MODE_2_DEPTH) && cur_depth > 2)
	{
		return false;
	}
...
void draw_room(CHAR_DATA *ch, const ROOM_DATA *room, int cur_depth, int y, int x)
{
	...
	if (world[ch->in_room] == room)
	{
		...
		if (map_check_option(MAP_MODE_MOBS_CURR_ROOM))
		{
			draw_mobs(ch, ch->in_room, y, x);
		}
		if (map_check_option(MAP_MODE_OBJS_CURR_ROOM))
		{
	...
	for (int i = 0; i < NUM_OF_DIRS; ++i)
	{
			...
			&& (!EXIT_FLAGGED(room->dir_option[i], EX_HIDDEN) || IS_IMMORTAL(ch)))
		{
			...
			if (cur_depth == 1
				...
				&& (map_check_option(MAP_MODE_MOBS) || map_check_option(MAP_MODE_PLAYERS)))
			{
...
void print_map(CHAR_DATA *ch, CHAR_DATA *imm)
{
	...
	if (map_check_option(MAP_MODE_DEPTH_FIXED))
	{
		if (map_check_option(MAP_MODE_1_DEPTH))
		{
			fixed_1 = true;
		}
		if (map_check_option(MAP_MODE_2_DEPTH))
		{
			fixed_1 = false;
			fixed_2 = true;
		}
...
void Options::parse_menu(CHAR_DATA *ch, const char *arg)
{
	...
	{
		//ch->desc->map_options.reset(); // prool
...
ACMD(do_map)
{
	...
	{
		//ch->map_text_olc(argument); // prool
	}
}
