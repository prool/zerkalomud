...
if %actor.id% == %quest619.id%
  if %object.vnum% == 61901
    ...
    дум
    wait 1s
    * у доспеха и так два места лоада
    *if ((%random.1000% <= 30) && (%world.curobjs(3368)% < 1))
    *  mecho  &WСемаргл снял прекрасного вида доспех и протянул его Вам.&n
    *  wait 1s
    *  mload obj 3368
    *  г Вот завалялось тут у меня кой-чего, на, забирай.
    *  дать доспех %actor.name%
    *else
    if ((%random.1000% <= 30) && (%world.curobjs(3367)% < 2))
      mecho  &WСемаргл создал прекрасного вида браслет и протянул его Вам.&n
      wait 1s
      mload obj 3367
      г Вот тут волхвы научили меня руками трясти. Иногда получается что-то...
