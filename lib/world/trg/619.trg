...
0 j 100
~
if (%actor.name% == %quest619.name%)
if %object.name% == голова змея 
wait 10
mpurge %object.name%
ул
...
0 j 100
~
if %object.name% == кусок застывшей лавы 
wait 10
mpurge %object.name%
...
wechoaround %actor% _%actor.name% пнул%actor.q% стену, разбивая непрочные камни.
...
~
mload obj 61953
if (%world.curobjs(12121)% < 1) && (%random.100% <= 2)
   mload obj 12121
end
~
#61914
