* BRusMUD trigger file v1.0
#61400
1я просьба безногого~
0 0 100
~
wait 4
makeuid questor614 %actor.id%
worlds questor614
%actor.setquest(61400)%
г Брат мой, Митька, просит о помощи. Но я без ног и помочь ему не могу.
wait 3
г Помрет Митька. Не откажи добрый молодец в помощи.
wait 3
г  Живет он тут рядом. На поляне, что на западной стороне леса; у самой реки.
calcuid room61446 61446 room
attach 61410 %room61446.id%
exec 61410 %room61446.id%
attach 61402 61408 mob
wait 3
г Как спасешь братка моего, забеги ко мне, я тебя медком угощу.
detach 61400 %self.id% 








~
#61401
благодарность слепого~
0 q0 100
~
 if !(%questor614.id% == %actor.id%) then
   msend %actor% Богатырь сдержано улыбнулся.   
halt 
end
if  !(%killer614.id% == %questor614.id%) then
    msend %actor% Богатырь вздохнул.
halt
end
wait 4
say Спасибо тебе огромное добрый молодец. Медведь прям взбесился,
say Задрал бы меня если б не ты.
say Это брат мой, Колька, тебя прислал?  Да не отвечай, я и сам знаю, что он.
wait 1s
say Добрый и отзывчивый ты человек, держи вот в благодарность все мои сбережения.
 say И не серчай на инвалида умственного труда.
wait 4
say Богатырь дал Вам маленький мешочек с деньгами.
%actor.gold(+500)%
makeuid questor614 %actor.id%
worlds questor614
calcuid mob61401 61401 mob
attach 61403  %mob61401.id%
detach 61401 %self.id% 









~
#61402
смерть медведя~
0 f0 100
~
makeuid killer614 %actor.id%
calcuid questor 61408 mob
worlds killer614
calcuid slepoy 61400 mob
attach 61401 %slepoy.id%
detach 61402 %self.id%









~
#61403
2й подход к безногому~
0 q0 100
~
if !(%questor614.id% == %actor.id%) then
   wsend %actor% МУДИЛО!!!
halt
end
wait   2
emote страшно обрадовался при вашем появлении 
say  Спасибо тебе, что братка моего выручил... Кхм...  Кхм...
wait 3
say Мне очень неудобно просить тебя повторно, но выбора у меня не наблюдается.
wait 2
say  Есть у меня к тебе еще одна просьба. Понимаешь ситуация в округе стала резко 
wait 2
say Ухудшаться последнее время. То ребенок в лесу пропадет, то стая диких и кравожадных ворон налетает на деревню.
wait 2
say Тут и медведь еще этот бешеный объявился черт знает откуда.
wait 2
say Мы с братом думаем, что это все проделки лесной ведьмы, старой карги- бабки Яги. 
wait 1s
say Может поможешь нам одолеть эту напасть? А уж мы за оплатой не постоим.
attach 61404 %self.id%
detach 61403 %self.id%







~
#61404
2й подход к безногому(часть 2)~
0 d0 0
конечно помогу да~
 if  !(%questor614.id% == %actor.id%)
   msend %actor% - Куда ты лезешь? Не с Тобой разговаривают!!
   halt 
end
wait   4
say Отлично. Ведьма  живет где-то в самой глуши. Но просто найти ее избушку мало.
wait 2
say Ведь в избушку так просто не попасть. Там слова магические нужны.
wait 2
say Слова эти расскажет тебе лишь тот кто знает все обо всех, а именно корчмарь в местном кабаке.
wait 2
say Но будь осторожен когда пойдешь к нему, народ там лихой, да и подраться с незнакомцем 
wait 2
say Случая не упустят, так что держи ухо в остро.
wait 2
say Вообщем когда доберешься до ведьмы, то не убивай ее сразу.
wait 2
say Просто надери ее старческую задницу, а когда она взмолится о пощаде, то требуй чтоб она 
wait 2
say Тебя к колодцу с живой водой отвела.
wait 2
say Воды той набери да неси ее мне, даст Бог излечусь я - отращу себе новые молодые ноги.
wait 2
say Буду бегать как степной скакун...
wait 2
say Ну бывай, удачи тебе. Я буду ждать тебя в хижине у брата.
wait 1s
calcuid mob61411 61411 mob
attach 61405 %mob61411.id%
calcuid room61447 61447 room
attach 61411 %room61447.id%
exec  61411 %room61447.id%
mecho Безногий богатырь, смешно виляя ягодицами, уполз на восток.
mpurge безног.бог
detach 61404 %self.id% 











~
#61405
подход к корчмарю~
0 q0 100
~
 if !(%questor614.id% == %actor.id%))
   halt 
end
г Ты что-то ищешь друг?
attach 61406 %self.id%
detach 61405 %self.id%









~
#61406
разговор с корчмарем(1)~
0 d0 0
да конечно ищу~

 if  !(%questor614.id% == %actor.id%)
   msend %actor% - Куда ты лезешь? Не с Тобой разговаривают!!
   halt 
end
say  А что ты ищешь? Хочешь узнать что-то?
say Любая информация стоит денег...
wait 2
say Я думаю за 100 кун я смогу найти ответ на любой твой вопрос.
wait 2
attach 61407 %self.id%
detach 61406 %self.id%









~
#61407
разговор с корчмарем(2)~
0 m0 1
~
if !(%questor614.id% == %actor.id%)
   msend %actor% - Спасибо тебе большое прибольшое, щедрый ты человек.
   halt 
end
if  %amount% < 100 then
switch %random.6% 
case 1
ухм
г Меньше знаешь - дольше живешь.
mecho Корчмарь отвернулся и принялся протирать кружку.
break
case 2
эмоц усмехнулся
г Лучше бы тебе вообще туда не ходить.
mecho Корчмарь отвернулся и принялся протирать кружку.
break
case 3
улыб
г Не скупись, накинь еще сверху.
mecho Корчмарь отвернулся и принялся протирать кружку.
break
case 4
mecho Говорит очень неразборчиво
г а... про... это ... туд... "От.. я ...."
mecho Корчмарь отвернулся и принялся протирать кружку.
break
case 5
г Деньги давай, давай деньги.
mecho Корчмарь отвернулся и принялся протирать кружку.
break
case 6
г Может тебе еще и пивка налить?
mecho Корчмарь отвернулся и принялся протирать кружку.
break
else
 if %amount% > 99 then
 switch %random.2% 
case 1
г Говрить тебе я ничего не буду, потомучто ты не запомнишь.
г Лучше я тебе все на бумажке напишу
г Как дойдешь до избушки ведьминой, так сразу читай вслух эти слова.
mload obj 61414
дать записка %actor.name%
calcuid room61460 61460 room
attach 61408 %room61460.id%
wsend %actor% %room61460.id%
attach 61408 %room61460.name%
detach 61407 %self.id%
break
case 2
г Информация - страшная сила. Кто  вовремя получает нужную информацию, тот правит миром.
г Поэтому за то что ты хочешь знать я прошу еще 100 кун.
halt
break
 end
end


~
#61408
читаем бумажку~
2 c0 0
читать прочитать зачитать дикломировать~
if !(%arg.contains(бумажка)) then
wsend %actor.name% Что вы хотите продикломировать ?  
 return 1
   halt
end
 if  !(%questor614.id% == %actor.id%) then
   msend %actor% Вы громко начали читать слова из записки, но  вскоре сбились из-за неразборчивого подчерка. 
mechoaround %actor% %actor.name% начал%actor.g% громко читать слова по бумажке, но сбил%actor.u% на
mechoaround %actor% третьем слове и продолжить уже не смог%actor.q%.
return 1
   halt 
end
wsend %actor.name% Вы развернули записку корчмаря и начали громко читать.
wait 1s
wforce %actor.name% г Избушка, избушка,
wait 1s
wforce %actor.name% г Встань ко мне передом,
wait 1s
wforce %actor.name% г К лесу задом.
wait 3s
wecho Избушка сказала: 'Да иди ты, я на это больше не ведусь.'
wait 4s
wforce %actor.name% г Избушка, избушка
wait 1s
wforce %actor.name% г Встань ко мне передом
wait 1s
wforce %actor.name% г К лесу задом.
wait 3s
wecho Избушка сказала: 'Сказано же тебе, вали отседова.'
wait 1s
wecho Избушка сказала: 'И почему сюда все время, какие-то ущербные на голову люди заходят?'
wait 4s
wforce %actor.name% г Избушка, избушка,
wait 1s
wforce %actor.name% г Встань ко мне передом,
wait 1s
wforce %actor.name% г К лесу задом.
wait 3s
wecho Избушка сказала: 'Ах ты - черт упертый!'
wecho Избушка сказала: 'Ну ладно так уж и быть.'
wait 1s
wecho Избушка начала медлено поворачиваться.
wait 2
wforce %actor.name% брос бумажк.магич.слов
wecho Бумажка упала на землю и обратилась в пепел. 
wpurge бумажк.магич.слов
wpurge 61414
wait 1s

wdoor  61460  east room 61461 
wdoor 61461 west room 61440

detach 61408 %self.id%







~
#61409
смотрим на избу~
2 c0 0
смотреть осмотреть оглядеть~
if !%arg.contains(изба)
wsend %actor.name% Что вы хотите тут осмотреть ?  
 return 1
   halt
end
wsend %actor% Входа в избу не видать, похоже он с обратной стороны, но как же туда попасть?
wechoaround %actor% %actor.name% внимательно оглядел%actor.g% избу.









~
#61410
лоад медведя~
2 0 100
~
wload mob 61408










~
#61411
REлоад безногого~
2 0 100
~
wait 2s
calcuid mob61401 61401 mob
wload mob 61401
wait 2s
attach 61427 %mob61401.id%
detach 61400 %mob61401.id%
detach  61411 %self.id%







~
#61412
репоп зоны~
2 f0 100
~
unset questor614
unset killer614








~
#61413
пустой тригер~
0 q0 100
~
if  !(%questor614.id% == %actor.id%)
   attach 61400 %self.id%
exec 61400 %self.id%
 detach 61413 %self.id%
end
wait 3
ждать %questor614.name%
attach 61427 %self.id%
detach 61413 %self.id%






~
#61414
смерть бабы яги~
0 f0 100
70~
calcuid room61464 61464 room
attach 61415 %room61464.id%
exec 61415 %room61464.id%
detach 61414 %self.id%







~
#61415
мольба яги~
2 0 100
~
wait 2s
wecho Труп Бабы Яги дернул ногой, похоже Яга еще не умерла.
wait 1s
wload mob 61417
mload mob 61417
wecho  Баба яга зашевелилась и начала подниматься на четвереньки.
wait 3
wecho Яга заметила вас!
wait 2
wecho Баба Яга старая карга с криком завалилась на пол.
wait 4
wecho Баба Яга старая карга стала молить о пощаде
wait 2
wecho Баба Яга сказала: 'Пощадите меня, я для вас все все сделаю! Отпустите бедную старушку!
calcuid mob61417 61417 mob
attach 61416 %mob61417.id%
exec 61416 %mob61417.id%
detach 61415 %self.id%







~
#61416
предложение яги~
0 0 100
~
wait 1s
эм хитро прищурилась
wait 2
г Знаю я местечко тайное.
wait 4
г Там колодец волшебный стоит, и вода в нем не простая, а волшебная:-)
wait 4
г Если отпустишь меня, то я покажу тебе это место.
wait 2
зевать
attach 61417 %self.id%
detach 61416 %self.id%  







~
#61417
предложение яги(2)~
0 d0 0
веди отведи покажи показывай~
wait 3
msend %actor%  Яга схватила вас за руку и потащила за собой на выход из избушки.
mechoaround %actor% Баба Яга схватила %actor.rname% за руку и потащила за собой к выходу.
wait 1s
mecho Вы продираетесь сквозь лесную чащу вслед за Бабой Ягой.
wait 6 
mecho Вы вышли на маленькую лесную полянку.
mteleport all 61477
calcuid room61477 61477 room
attach 61418 %room61477.id%
exec 61418 %room61477.id%
mpurge баба.яга
detach 61417 %self.id%






~
#61418
(!!!О господи что я делаю!!!)~
2 0 100
~
wload mob 61417
calcuid mob61417 61417 mob
attach 61419 %mob61417.id%
exec 61419 %mob61417.id%
detach 61418 %self.id%







~
#61419
последние слова яги~
0 0 0
~
wait 1s
г Вот он колодец волшебный.
wait 3
г Когда напьешься в доволь, то иди на запад и выйдешь к самой деревне.
mecho Баба Яга весело хихикая умчалась в чащу леса.
mpurge баба.яга
detach 61418 %self.id%







~
#61420
лезем в печь~
2 c0 0
лезть залезть~
if !%arg.contains(печь)%
wsend %actor.name% Куда вы хотите залезть ?  
 return 1
   halt
end
wait 5
wsend   %actor.name% Вы открыли заслонку печи и полезли внутрь.
wechoaround %actor% %actor.name% открыл%actor.g% заслонку печи и быстро полез%actor.q% в печь.
wait 1s
wteleport %actor% 61462






~
#61421
лезем в пень~
2 c0 0
открыть ~
if !%arg.contains(пень)%
wsend %actor.name% Что вы хотите открыть ?  
 return 1
   halt
end
wait 4
wsend   %actor.name% Вы начали ощупывать пенек в поисках места за которое можно уцепиться и открыть люк.
wechoaround %actor% %actor.name% с видом профессионала начал%actor.g% трогать пень.
wait 4
wechoaround %actor% %actor.name% завизжал%actor.g%, как свинья перед родами и ...
wsend   %actor.name% очень скоро вы нащупали маленький паз и с криком откинули деревянный люк.
wechoaround %actor% ... Откинул%actor.g% деревянный люк.
wdoor 61452 down room 61470 
wdoor 61470 up room 61452
detach 61421 %self.id%






~
#61422
реакция кузнеца~
0 q0 100
~
wait 4
хму %actor.name%

~
#61423
слова старосты~
0 r0 100
~
mload mob 61424
mload mob 61424
wait 3
г Мать твою !! Что за канитель?!?
wait 3
г Ты как сюда пробрался ворюга мерзкий?!!
wait 2
г Стража! Стража ! Хватайте вора.
wait 2
mecho Стражник прибежал на крики старейшины.
wait 1
mecho Стражник прибежал на крики старейшины.
detach 61423 %self.id%

~
#61424
реакция охраны~
0 q0 100
~
wait 2
г Стой, кто идет?!?
wait 3
г Поворачивай назад, старейшина просил его не беспокоить.




~
#61425
выпивоха~
0 b0 100
~
wait 3
эм спусти штаны и начал испоражняться прямо под себя.






~
#61426
старый алкаш~
0 b0 100
~
г Водки! Неси Водки.
руг






~
#61427
выдача наград~
0 j0 100
~
if !(%object.vnum%==61400) then
wait 4
г Зачем мне это??
wait 2
г  Я в воде живой нуждаюсь.
wait 2
взд
halt
end
if %object.vnum%==61400 then
  if !(%questor614.id% == %actor.id%) then
   msend %actor% Богатырь сдержано улыбнулся.   
wait 1s
г Спасибо тебе добрый молодец.
wait 3
mecho Безногий богатырь отхлебнул пару раз из жестяного ведра.
wait 4
mecho Вдруг богатырь задергался и прямо у вас на глазах у него выросли ноги.
wait 3
mecho Богатырь встал на ноги и попрыгал.
wait 3
mecho "Безногий" богатырь передал жестяное ведро слепому брату.
wait 4
mecho Слепой богатырь с жадностью припал к ведру.
wait 4
mecho "Слепой" богатырь прозрел и засветился от счастья.
halt 
end
wait 3
say Не может быть !!
wait 4
г  Я не верю своим глазам.
wait 1
удивл %questor614.name%
wait 4
г Ты сделал это ?!?!!
wait 4
г Сейчас проверим, что ты нам принес ...
wait 2
mecho Безногий богатырь отхлебнул пару раз из жестяного ведра.
wait 1s
г Э-э...
wait 3
г Похоже ты вздумал меня обмануть!!!?!!!
wait 4
г Что это ты мне подсунул, где моя живая вода???!!!
wait 3
mecho Вдруг богатырь задергался и прямо у вас на глазах у него выросли ноги.
wait 4
mecho Богатырь встал на ноги и попрыгал.
wait 3
mecho Богатырь добродушно улыбнулся.
wait 5
г Прости, что не поверил тебе %questor614.name%, Спасибо тебе огромное.
wait 4
mecho "Безногий" богатырь передал жестяное ведро слепому брату.
wait 4
mecho Слепой богатырь с жадностью припал к ведру.
wait 3
mecho "Слепой" богатырь прозрел и засветился от счастья.
wait 4
г Велики твои подвиги %questor614.name%, и мы врят ли сможем когда-нибудь в полной мере отблагодарить тебя.
wait 5
г  Возьми пожалуйста все наши сбережения в знак благодарности и уважения.
wait 2
%self.gold(+2000)%
дать 2000 кун %questor614.name%
end
detach 61427 %self.id%

~
#61428
набор воды~
2 c0 0
набрать достать~
if !%arg.contains(живая вода)%
wsend %actor.name% Что вы хотите набрать ?  
 return 1
   halt
end
wait 4
wsend %actor% Путем не сложных операций по спуску и подъему ведра в глубокий колодец вы все же набрали полное ведро живой воды.
wechoaround %actor% %actor.name% набрал%actor.g% живой воды из лесного колодца.
wload obj 61400
wforce %actor.name% взять ведро.






~
#61429
лягуха~
0 b0 100
~
wait 4
г КВА - КВА !






~
#61430
лезем в щель(в печь снизу)~
2 c0 0
лезть залезть пролезть~
if !%arg.contains(щель)%
wsend %actor.name% Куда вы хотите залезть ?  
 return 1
   halt
end
wait 5
wsend   %actor.name% Вы ухватились за край потолочной щели и ловко пролезли вверх.
wechoaround %actor% %actor.name% ухватил%actor.u% за край потолочной щели и пролез%actor.u% наверх.
wait 1s
wteleport %actor% 61462



~
#61431
лезем из печи~
2 c0 0
вылезти лезть ползти~
if !%arg.contains(назад)%
wsend %actor.name% Куда вы хотите залезть ?  
 return 1
   halt
end
wait 5
wsend   %actor.name% Вы открыли заслонку печи и полезли наружу.
wait 1s
wteleport %actor% 61463
wechoaround %actor% %actor.name% открыл%actor.g% заслонку печи и быстро вылез%actor.q% из печи.



~
#61432
взмах крыльев~
1 j0 100
~
wait 3
oechoaround %actor% %actor.name% поднял%actor.u% ввоздух от взмахов больших черных крыльев за его спиной.
osend %actor% Черные крылья затрепетали у вас за спиной, подняв вас над землей.



~
#61433
прыгаем из печи~
2 c0 0
прыгнуть спрыгнуть~
if !%arg.contains(щель)%
wsend %actor.name% Куда вы хотите спрыгнуть ?  
 return 1
   halt
end
wait 5
wsend   %actor.name% Вы спрыгнули вниз.
wait 1s
wteleport %actor% 61475
wechoaround %actor% %actor.name% шлепнул%actor.u% сверху.



~
#61434
думки мужика~
0 b0q0 100
~
wait 4
дум



~
#61435
приветствие нищего старика~
0 q0 100
~
makeuid proklyatiy614 %actor.id%
worlds proklyatiy614 
attach 61437 %proklyatiy614.id%
mecho прокл614    %proklyatiy614.id%
mecho  фктор     %actor.id%
wait 2
г Здраве буде добрый молодец.
wait 1
г Помоги мне бедному, видишь я стар и поколечен.
wait 2
г Родных нет, никому я не нужен.
плак
рыд
г Не откажи в помощи, дай мне 50 кун, ведь я тебе в отцы гожусь.
attach 61436 %self.id%
detach 61435 %self.id%

~
#61436
даем денег старику~
0 m0 1
~
if  %amount% > 49  then
wait 3
г Спасибо сынок, ступай теперь своей дорогой, а я пойду своей.
mecho Старик взмахнул рукой.
detach 61437 %actor.id%
dg_cast 'сила' %actor.name% 
dg_cast 'защита' %actor.name% 
dg_cast 'полет' %actor.name% 
улыб
unset proklyatiy614
attach 61435 %self.id%
detach 61436 %self.id%
else
if  %amount% < 50  then
wait 2
хму
wait 3
г Пожадничал для дедушки?!? 
wait 2
г Да будь ты проклят, сучий ты сын!
attach 61437 %actor.id%
attach 61435 %self.id%
detach 61436 %self.id%
end
end

~
#61437
проклятие(на персоонажа)~
0 b0 100
~
switch %random.6%
case 1
wait 20s
mechoaround %self.id% %self.name% подскользнул%self.u% и упал%self.g%.
г Черт побери!!!
 сесть
break
case2
wait 20s
г Да пошли вы все на хрен !!!
 бежать
 бежать
 бежать
break
case 3
 wait 20s
пук
break
case 4
wait 20s
зев
спать
храп
break
case 5
wait 20s
зачит возв
break
case 6
wait 20s
снять все
break
end

~
#61438
смерть старика~
0 f0 100
~
detach 61437 %actor.id%


~
#61439
вид с холма~
2 c0 100
осмотреться оглядеться~
wait 5
wsend %actor% С холма открывается прекрасный вид на окресности.
wsend %actor% На север простирается густой лес.
wsend %actor% На западе с юга на север течет река, а за рекой все покрыто непроглядным туманом.
wsend %actor% На юге лежит деревушка, а за ней лесная дорожка, направляющаяся на юг.

~
#61440
смерть медведя~
0 f0 100
~
if (%random.100% < 20) && (%world.curobjs(61403)% < 5)  then
mload obj 61403
end

~
#61441
смерть варга~
0 f0 100
~
if (%random.100% < 15) && (%world.curobjs(61412)% < 3)  then
mload obj 61412
end

~
#61442
смерть волка~
0 f0 100
~

if (%random.100% < 50) && (%world.curobjs(61405)% < 8)  then
mload obj 61405
end

~
#61443
смерть лисы~
0 f0 100
~
if (%random.100% < 25) && (%world.curobjs(61407)% < 5)  then
mload obj 61407
end

~
#61444
смерть ворона~
0 f0 100
~
if (%random.100% < 30) && (%world.curobjs(61415)% < 5)  then
mload obj 61415
end
if (%random.100% < 10) && (%world.curobjs(61419)% < 3)  then
mload obj 61419
end

~
#61445
смерть филина~
0 f0 100
~
if (%random.100% < 40) && (%world.curobjs(61411)% < 6)  then
mload obj 61411
end

~
#61446
лоад охотника~
0 n0 100
~
if (%random.100% < 50) && (%world.curobjs(61409)% < 5)  then
mload obj 61409
end
if (%random.100% < 15) && (%world.curobjs(61416)% < 5)  then
mload obj 61416
end
if (%random.100% < 15) && (%world.curobjs(61413)% < 5)  then
mload obj 61413
end

~
#61447
лоад охранника~
0 n0 100
~
if (%random.100% < 10) && (%world.curobjs(61406)% < 5)  then
mload obj 61406
end

~
#61448
лоад стража~
0 n0 100
~
if (%random.100% < 20) && (%world.curobjs(61420)% < 5)  then
mload obj 61420
end

~
#61449
лоад мальчугана~
0 n0 100
~
if (%random.100% < 15) && (%world.curobjs(61410)% < 5)  then
mload obj 61410
end

~
#61450
лоад кузнеца~
0 n0 100
~
if (%random.100% < 10) && (%world.curobjs(61408)% < 3)  then
mload obj 61408
end

~
#61451
лоад алкаша~
0 0 100
~

if (%random.100% < 30) && (%world.curobjs(61417)% < 10)  then
mload obj 61417
end

~
#61452
лоад бродяги~
0 n0 100
~
if (%random.100% < 20) && (%world.curobjs(61404)% < 6)  then
mload obj 61404
end

~
#61453
лоад яги~
0 n0 100
~
if (%random.100% < 15) && (%world.curobjs(61418)% < 5)  then
mload obj 61418
end

~
#61454
лоад селянки~
0 n0 100
~
if (%random.100% < 5) && (%world.curobjs(61421)% < 5)  then
mload obj 61421
end

~
$
$
