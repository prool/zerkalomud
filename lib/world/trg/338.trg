...
if !((%world.curobjs(33821)% == 1) && (%world.curobjs(33821)% == 1))
   wsend       %actor% Нельза смешать что то если этого у вас нет.
   return 0
   halt
end
  wsend       %actor% Вы насыпали в руку немного белого и немного серого порошков.
  wechoaround %actor% %actor.name% начал%actor.q% объединять белый и серый порошки.
  ...
  calcuid piosik 33827 mob
  wload obj 33823
  wecho _Поднялся черный дым и, когда он рассеялся, вы увидели черный порошок.
~
#33812
...
mecho _Священник быстро ушел куда то.
calcuid rorik 33800 mob
...
mechoaround %actor% Воевода недоверчиво осмотрел %actor.rname%.
...
if !(%arg.contains(яма)%) 
  ...
  wechoaround %actor% %actor.name% прыгнул%actor.q% в яму.
  wait 1s
  wsend %actor% .- Вы почуствовали запах гнили.
  wteleport %actor.name% 33884
  wat 33884 wechoaround %actor% Кто то спрыгнул сюда сверху.
~
#33817
вылезеш из ямы с медведем~
2 c 0
лезть пролезть залезть прыгнуть~
...
mechoaround %actor% _Набажавшая стража схватила %actor.vname% и утащила невесть куда.
...
mteleport %actor.name% 33880
wait 1s
mteleport труп 33893
end
~
#33821
помер Хан~
