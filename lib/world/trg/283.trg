...
%echo% Зря вы ее оторвали от трапезы.
~
#28306
...
if (%actor.class% == 2 || %actor.class% == 4) 
   ...
   wait 2
     say У нас тут появились проблемы с властями, и я бы заплатил тебе за помошь в их решении
     say Возьмешся нам помочь?
   wait 2
...
if (%actor.name%==%rkvester%)
 if (%arg%==колодец)
  if (%actor.eq(17)% != %сверток%)
   %send% %actor% Вы незаметно бросили сверток в колодец.
   %purge% отрава
   %send% %actor% Теперь пора и за платой....
      eval rkvest yes
      global rkvest yes
  else
   %send% %actor% И чем травить собрались?
  end
 else
  %send% %actor% Чего травить будем?
 end
end
~
#28312
...
if (%actor.name%==%rkvester%)
 if (%rkvest%==yes)
  ...
  say Ну типа спасибо, а за хлопоты я уж необижу.
  say Держи.
  say И если понадобится скрыться от властей заходи.
  say Укроем, конечно не за так, но много невозьмем, да и работенку подкинем.
    set %actor.gold(+300)%
    eval blag %actor%
    global blag
    calcuid glav 28312 mob
...
if (%actor.class% == 2 || %actor.class% == 4) 
   ...
   wait 2
     ...
     msend %actor% %self.name% хлопнул Вас по плечу
   wait 2
     say У нас тут появились проблемы с купцами, и я бы заплатил тебе за помошь в их решении
     say Возьмешся нам помочь?
   wait 2
     msend %actor% %self.name% вопросительно уставился на вас.
     calcuid glav 28324 mob
...
say Ну а коли не снимеш, то уж необессуть, делиться нечем будет.
   set %rkvester% %acto.name%
   global rkvester
 calcuid glav 28324 mob
...
if (%actor.name%==%rkvester%)
 ...
 wait 5
  say Ась?
 wait 5
 say А ну свалил отсюда чтоб глаза мои тебя невидели!
 wait 2s
...
if (%actor.name%==%rkvester%)
  ...
  say Неплохо поработал, неошибся я в тебе!
  say Держи.
  say И если понадобится скрыться от властей заходи.
  say Укроем, конечно не за так, но много невозьмем, да и работенку подкинем.
    set %actor.gold(+300)%
  ...
  say Коли несправился с заданием так и говори, а надуть меня даже не пытайся!
  end
end
~
#28320
...
if (%actor.class% == 5 || %actor.class% == 6)
   ...
   say Приветствую тебя,  %actor.name%!
   say Хорошо что зашел ко мне, у меня есть предолжение для тебя.
