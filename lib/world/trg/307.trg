...
wait 1s
mecho _Медведь громко отхлебнул из %object.rname% и расплылся в широкой улыбки.
wait 1s
...
wait 1s
mecho _Медведь взял %object.name% под мышку и прощально помахал всем рукой.
wait 1s
...
wait 1s
говор Когда спасешь лес, приходи ко мне и потолкуем о спасение белок!
...
if (!%self.fighting%)
msend       %actor% _Вы залезли под брюхо толстой тли и начали ее гладить.
mechoaround %actor% _%actor.name% залез%actor.q% под брюхо толстой тли и начал%actor.g% ее гладить.
wait 1s
...
if (!%self.fighting%)
wait 1s
mecho _Вдруг худая тля перестала жевать и ее брюхо начало раздуваться.
wait 1s
mjunk all
mload mob 30702
mecho _Худая тля чуть присела на задние лапы, превратившись в толстую.
wait 1
mpurge %self%
end
~
...
0 ab 20
~
if %world.curmobs(30703)% <3
wait 1s
mecho _Тля присела на задние лапы и начала тужиться. 
wait 1s
mecho _С диким криком тля родила маленького мохнатого тленка. 
mload mob 30703
end   
if %random.100% < 5
...
eval ypal %random.3%
if %ypal% == 2
wsend       %actor% _Не устояв на шатких камнях, Вы больно упали, разбив об огромный валун свой прекрасный нос.
wechoaround %actor% _Не устояв на шатких камнях, %actor.name% больно упал%actor.g%, разбив об огромный валун свой прекрасный нос.
wdamage %actor% 20
elseif %leshiy% ==1
wsend       %actor% _Не удержавшись на камнях, Вы с криком полетели вниз, сильно ободрав руки и ноги.
wechoaround %actor% _Не удержавшись на камнях, %actor.name% с криком полетел%actor.g% вниз, сильно ободрав руки и ноги.
wdamage %actor% 40
wforce %actor.name% сесть
elseif %leshiy% ==0
wsend       %actor% _Вы споткнулись об небольшой камень и вывернули себе лодыжку.
wechoaround %actor% _%actor.name% споткнул%actor.u% об небольшой камень и вывернул%actor.g% себе лодыжку.
wdamage %actor% 30
else
wsend       %actor% _Поскользнувшись, Вы упали лбом на огромный валун, от чего тот разлетелся вдребезги.
wechoaround %actor% _Поскользнувшись, %actor.name% упал%actor.g% лбом на огромный валун, от чего тот разлетелся вдребезги.
wdamage %actor% 15
end
end
~
...
0 j 100
~
if %object.vnum% == 30809 then
  wait 1s
  говор Белочки мои! Родненькие!
  wait 1s
  mecho _Лохматый Леший бросил связку на землю и сделал загадочный жест.
  wait 1s
  mecho _Белки выросли у Вас на глазах.
  mload mob 30813
  mload mob 30813
  mload mob 30813
  wait 1s
  mecho _Лохматый Леший ласково потрепал их за хвост.
  wait 1s
  говор Ну, а теперь бегите в лес.
  mecho _Зубастые белки выбежала из берлоги.
  ...
  eval nagrom %random.50%
  if (%world.curobjs(30717)% < 5) && (%nagrom% > 25)
   mecho  _Леший снял с себя старый кафтан и протянул его Вам.
   mload obj 30717
   дат кафт %actor.name%
   elseif (%world.curobjs(30715)% < 3) && (%nagrom% < 25)
   mecho  _Леший вытащил из-за пазухи бересту и протянул ее Вам.
   mload obj 30718
   дат берест %actor.name%
   else
   mecho  _Леший вытащил из-за пазухи мешочек и протянул его Вам.
   mload obj 30716
   дат мешоч %actor.name%
end
