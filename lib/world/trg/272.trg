...
msend %actor% подмоги.
msend %actor% Я чем смогу, тем тебя отблагадарю.
~
#27201
умерповелитель~
0 f 100
...
if (%actor.class% == 5) && (!%actor.skill(веерная защита)%)
 if (%actor.level%>=24)
  ...
  mechoaround %actor% Старик посмотрел на %actor.rname%.
  msend %actor% Ты еще мал чтобы получить данное умение, но я отблагадарю тебя.
 end
end
* витязь
...
~
wait 1
mecho %actor.name% Глаза испуганного жителя на мнговение остановились на Вас.
mecho %actor.name% Вдруг он заорал: ужас, ужас, бегите отсюда, там
хих
wait 1
хох
wait 1
mecho %actor.name% там ужас, там смерть, всем смерть.
mecho %actor.name% Так же быстро испуганный житель отвернулся от вас и резко отпрыгнул от вас.
~
$~
