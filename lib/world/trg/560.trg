...
wait 1s
wecho Вы услышали какую-то возьню за воротами..
wait 1s
...
wsend %actor% Что вы хотите спустить вниз?
~
#56005
приветствие ивасек~
0 r 100
...
wait 1s
   say Вы наверное видили уже высокий терем, что стоит  в южной части двора крепости, 
...
if (%arg.contains(ковер)%)
   ...
   wechoaround %actor% %actor.name% приподнял%actor.g% один из ковров и Вы увидили за ним дверь!
...
0 r 100
~
if %actor% != %hero3% || !%world.curmobs(56009)%
halt
end
wait 2s
say Ну вот мы и свободны. Что ж пора нам вернуться домой.
