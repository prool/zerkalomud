...
halt
  ...
  mecho Старичок довольно закряхтел, увидев Вас.
  say  Если ты идешь на север, то берегись, там жуткие болота.
  say А я смог бы тебя проводить через них вполне безопасно.
  say Что бы хоть как то вознаградить мой тяжкий труд, не пожалей для
  say старика 40 кун.
~
#38101
у мужичка болотника~
...
~
  wait 1
  say Переправа через болота стоит 50 кун.
  say Если не хочешь платить - то иди туда сам%actor.g%
  ухм
~
#38102
дал бабки мужичку~
...
wait 1
msend   %actor%  Мужичек болотник поднялся и завязал  вам глаза тряпицею.
mechoaround %actor%  Мужичек болотник поднялся и завязал  %actor.dname%  глаза тряпицею.
msend   %actor% Он взял вас за руку и повел неведанными тропками.
mechoaround %actor%  %actor.name% и болотник ушли на север.
wait 1
...
if !%actor.rentable%
  ...
  calcuid wbozku 38129 room
  attach 38107 %wbozku.id%
  detach 38104 %wbozku.id%
~
#38105
репопим тригера~
2 f 100
~
calcuid wbozku 38129 room
attach 38104 %wbozku.id%
detach 38107 %wbozku.id%
if !(%random.25% == 1)
...
0 f 100
~
if (%world.curobjs(38101)% < 1) && (%random.10% == 1)
   mload obj 38101
end
~
#38115
