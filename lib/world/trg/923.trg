...
set target %random.pc%
mecho Яркие молнии ударили из фонтата!
...
0 k 100
~
if %random.20% == 1 && %world.curmobs(92322)%<1
...
0 j 100
~
if %object.vnum%91201>1 && %object.vnum%<91207 
...
0 k 100
~
if %random.40% == 1 && %world.curmobs(92322)%<1
mecho Скипер-зверь ударил копытом по полу!
mecho Сверкающие осколки вылетели из зеркал!
mload 92322 mob
mload 92322 mob
...
if %actor.skill(палицы)% < 141
mskilladd %actor.name% палицы 20
elseif if %actor.skill(короткие)% < 141
mskilladd %actor.name%  которкие 20
elseif %actor.skill(иное)% < 141
mskilladd %actor.name% иное 20
end  
  break         
  *воры
  case 2
...
mskilladd %actor.name% заколоть 20
elseif if %actor.skill(поднож)% < 141
mskilladd %actor.name%  поднож 20
elseif %actor.skill(украсть)% < 141
mskilladd %actor.name% украсть 20
end
  break   
  *батыры                        
  case 3 
if !%actor.skill(молот)%
mskillturn %actor.name% молот set
end
...
if %actor.skill(молот)% < 141
mskilladd %actor.name% молот 20
elseif if %actor.skill(оглушить)% < 141
mskilladd %actor.name% оглушить 20
elseif %actor.skill(ярость)% < 141
mskilladd %actor.name% ярость 20 
end
  break                          
  *наемы
  case 4
...
elseif if %actor.skill(поднож)% < 141
mskilladd %actor.name% поднож 20
elseif %actor.skill(скрытый)% < 141
mskilladd %actor.name% скрытый 20 
end
  break                       
  *друж
  case 5                          
...
if %actor.skill(осторож)% < 141
mskilladd %actor.name% осторож 20
elseif if %actor.skill(спасти)% < 141
mskilladd %actor.name% спасти 20
elseif %actor.skill(веер)% < 141
mskilladd %actor.name% веер 20 
end
  break  
  *витязь
  case 9 
...
if %actor.skill(блок)% < 141
mskilladd %actor.name% блок 20
elseif if %actor.skill(сбить)% < 141
mskilladd %actor.name% сбить 20
elseif %actor.skill(точный)% < 141
mskilladd %actor.name% точный 20 
end
  break                      
  *охотник
  case 10 
...
if %actor.skill(уклон)% < 141
mskilladd %actor.name% уклон 20
elseif if %actor.skill(луки)% < 141
mskilladd %actor.name% луки 20
elseif %actor.skill(дополнит)% < 141
mskilladd %actor.name% дополнит 20 
end
  break                        
  *кузнец
  case 11
...
mskilladd %actor.name% оглушить 20
elseif if %actor.skill(ярость)% < 141
mskilladd %actor.name% ярость 20
elseif %actor.skill(пнуть)% < 141
mskilladd %actor.name% пнуть 20 
end
  break                        
  *купец
  case 12
...
if !%actor.skill(маскиров)%       
mskillturn %actor.name% маскиров set
end
if !%actor.skill(осторож)%             
mskillturn %actor.name% осторож set
end
if %actor.skill(метнуть)% < 141
mskilladd %actor.name% метнуть 20
elseif if %actor.skill(маскиров)% < 141
mskilladd %actor.name% маскиров 20
elseif %actor.skill(осторож)% < 141
mskilladd %actor.name% осторож 20 
end
  break
  *волхв
...
if %actor.skill(палицы)% < 141
mskilladd %actor.name% палицы 20
elseif if %actor.skill(иное)% < 141
mskilladd %actor.name% иное 20
elseif %actor.skill(сглазить)% < 141
mskilladd %actor.name% сглазить 20 
end
  break
done
wait 2s
...
0 f 100
~
if %world.curobjs(92301)% < 4 && %random.4% == 1
mload obj 92301    
end
~
#92322
...
0 f 100
~
if %world.curobjs(92302)% < 4 && %random.4% == 1
mload obj 92302    
end
~
#92323
...
0 f 100
~
if %world.curobjs(92303)% < 4 && %random.11% == 1
mload obj 92303    
end
~
$~
