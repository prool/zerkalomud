...
oechoaround %actor% _%actor.name% взял%actor.q% одну из нитей паутины и начал%actor.q% ее дергать.
wait 1s
...
oechoaround %actor% _%actor.name% немного освежевал%actor.q% труп паука но мяса не нашел.
...
msend       %actor% _Вы прикоснулись к мышке.
mechoaround %actor% _%actor.name% нагло тронул%actor.q% беззащитную мышку.
wait 1s
...
0 f 100
~
if %world.curobjs(209)% < 30 && %random.4%==1
mload obj 209
end
~
#38527
...
osend  %actor% _Внезапно она стала удлинняться, выпрямляться и..
