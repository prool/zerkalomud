...
wait 1s 
say Говорил я вам, абалдуям, умерьте свой пыл!
say Дорвались до жилы - и сразу ум за разум зашел...
say На что вам эти безделушки сдалить только?!
wait 1s
эм покрутил в руках какой-то блестящий камушек
wait 1s
say Только поругались за зря. Да Полоз-покровитель на нас всех из-за вас гневается,
...
say Кто теперь за вас пойдет просить Полоза? Кто змей изничтожет? Ты, Дарен, чтоли?
mecho Каменетес Дарен понуро опустил взгляд...
wait 1s
say Или ты, Молчан?!
...
if %arg.contains(паутин)%
  ...
  oechoaround %actor% ~~%actor.name% начал%actor.g% рвать паутину.
  wait 1s
...
if (%arg.contains(мешочек)% || %arg.contains(мешок)%)
  if ( %random.1000% < 250 )
    ...
    wechoaround %actor% ~~%actor% присел%actor.q% на корточки и внимательно присмотрел%actor.u% к чему-то...
  else 
...
if ((%world.curobjs(243)% < 50) && (%random.1000% <= 180))
mload obj 243
end
