...
msend %damager% _Едва только вы схватились за оружие, как охранник кинулся на Вас и схватили за бока.
...
mechoaround %actor% _%actor.name% принес%actor.g% жертву богам.
...
mechoaround %actor% _%actor.name% вознес%actor.g% молитву.
...
if (%actor.clan% == тв)
if (%random.3% == 2)
msend %actor% _%self.name% вытнулся перед вами и отсалютовал оружием.
mechoaround %actor% _%self.name% вытнулся перед %actor.tname% и отсалютовал оружием.
покл %actor.name%
end
else
if %random.5% == 2
...
mecho _Лошадь поша в гости - а мужик стоял,
...
mecho _перенес меня мужик в черну избу,
...
elseif (%killer.sex% == 2)
mshou _Помогите! Убивает меня злодейка - %killer.name.%. Спаси...
else
mshou _Помоги....
end
...
halt
elseif (%sluh% > 2 )
msend %actor% - Недавно кметь какой-то урманский, пришлый, напишись начал на торговой площади буянить.
...
msend %victim% _Ромейский боей нанес вам несколько быстрых ударов и БОЛЬНО хлестнул по горлу!
...
msend %actor% дал вам несколько старных заморских монет.... - и что теперь с энтим богатством делать? 
