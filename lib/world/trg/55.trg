...
if (%object.vnum%==5500)&&!(%self.fighting%)
 ...
 attach 5507 %hostr.id%
  ...
  if (%world.curobjs(234)% < 50) && (%random.10% == 1)
   mload obj 234
   дать рун %actor.name%
  end
...
взять %vobj.name%
воор %vobj.name%
msend %actor.name% А я если что, встречу их здесь!
...
if %object.vnum%==5508
 ...
 mecho Хозяин глубокомысленно посмотел на вашу одежду, всю в пятнах крови.
