...
if %object.vnum% == 33105 
  ...
  wait 2s
    if (%world.curobjs(33110)% < 5) && (%random.4% == 1)
       mecho _Краб ловко разрезал клешней утробу и достал от туда водянистый лук..
       mload obj 33110
       wait 2s
       дат лук %actor.name%
    end
  if (%world.curobjs(206)% < 50) && (%random.3% == 3)
     ...
     mecho _Краб достал из под панциря руну света.
     mload obj 206
     wait 2s
     дат рун %actor.name%
  end
...
mecho _Вы услышали предсметрный вскрик!
mpurge %rusalk.id%
end
~
#33115
...
wechoaround %actor% %actor.name% с криком исчез%actor.g% в Водовороте!!! 
wait 1s
