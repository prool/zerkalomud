...
tell %actor.name% Привет добр человек, не будет ли у тебя пару минут.
...
tell %actor.name% будут тебе благоларны. Ну и я насколько это возможно.
...
tell %actor.name% Ты прошел, там больше нет этих гадких крыс?
tell %actor.name% Спасибо за освобождение!
tell %actor.name% Я тут книгами приторговываю, вот сюда на ночь залез,
tell %actor.name% не знал, что тут такие монстры гуляют.
tell %actor.name% Но могу Вам предложить свой товар.
tell %actor.name% Сегодня у меня с собой:
if %world.curobjs(517)%==0
tell %actor.name% 100  кун : ледяное прикосновение
end
if %world.curobjs(516)%==0
tell %actor.name% 500  кун : молчание
end
if %world.curobjs(512)%==0
tell %actor.name% 2000 кун : полет
end
if %world.curobjs(527)%==0
tell %actor.name% 200  кун : серьезное исцеление
end
if %world.curobjs(518)%==0
tell %actor.name% 400  кун : слабость
end
if %world.curobjs(538)%==0
tell %actor.name% 2500 кун : страх
end
if %world.curobjs(500)%==0
tell %actor.name% 1500 кун : кислота
end
if %world.curobjs(543)%==0
tell %actor.name% 1000 кун : серьезный вред
end
if %world.curobjs(537)%==0
tell %actor.name% 5000 кун : развеять магию
end
tell %actor.name% Сколько дадите то и продам.
tell %actor.name% Извиняюсь что прошу деньги, но я только усталый торговец книгами.
...
tell %actor.name% Знаешь, книга похоже пропала здесь в темноте или крысы унесли.
взд
...
дать 5000 кун %actor.name%
 ...
 break
default
tell %actor.name% Благодарствую.
ухм
 break
done
tell %actor.name% Все, я уношу поскорее отсюда ноги.
calcuid hmob 27613 mob
mpurge %hmob.name%
...
osend %actor.name% Похоже в детстве Вас бабушка учила обращаться с шерстью.
...
osend %actor.name% Ничего не получилось, шерсть пропала.
calcuid hobj 27613 obj
opurge %hobj.name%
end
~
