...
спросить~
context 794
if ((%actor.level% < 24) || (%actor.vnum% != -1))
...
wait 20
%actor.wait(1)%
wsend %actor% Ты уселся на весла и мощными гребками погнал лодку к небольшому фиорду,
...
*~
wait 1s
say Что, малыш%actor.q%, приш%actor.q% убить меня? 
оскал
wait 25
...
say побеспокоят. Спасибо тебе боец, ты избавил%actor.q% меня еще от одной проблемы.
wait 10
%actor.exp(+250000)%
wait 5
msend %actor% Вы заметили, что Ваш опыт увеличился.
...
Ну и шут с тобой. 
wait 25
г Короче. На острове белка. Мне мешает. Убей. Принеси мне труп. Понял%actor.q%, дерево?
...
wait 25
г Ну что видел%actor.q% белку? И откуда только это мерзкое животное берется?
...
context 794
wait 2s
say Спасибо, выручил%pman.q% ты меня! 
wait 10
