...
0 j 100
~
if %object.vnum% == 34604 then
  wait 2s
  say Благодарю Тебя, Боже! Воистину велика сила Твоя!!
  wait 2s
  mecho _Святой екзарх спрятал антиминс подальше.
  mpurge антиминс
...
дать посох %actor.name%
 ...
 %actor.exp(+400000)%
 msend За доброе дело Вы получили 400000 очков опыта.
end
 %actor.gold(+5000)%
 msend Святой екзарх дал Вам 5000 кун.
 mechoaround %actor% Святой екзарх дал большую груду кун %actor.dname%.
 пока
  end
  else
  say  Зачем мне это? 
...
say С горем пополам от них оборяемся...
пла
