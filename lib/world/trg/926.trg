...
oechoaround %actor% %actor.name% осмотрел замок,в его глазах мелькнула догадка!
oechoaround %actor% %actor.name% привычным движением потянулся за отмычкой.
oechoaround %actor% %actor.name% начал ковырятся отмычкой в замке.
...
say Какими бы могучими они не бьли -  это место не для живых.
...
set char %quester%
eval num %char.str%+%char.dex%+%char.con%+%char.wis%+%char.int%+%char.cha%
eval remort (92-%num%)/6
eval i 2+%remort%*3/4
if %i%>10
set i 10   
end
eval i %i%+9
if %random.20% > %i% && !%actor.quested(926)%
attach 92627 %self.id%
msend %char% Чему ты хочешь у меня научиться?
msend Если стиль это боевой, то скажи какой:
msend %char% осторожный
*msend %char% точный 
msend %char% _
msend %char% Иль навык какой хочешь приобрести?
msend %char% взломать
msend %char% украсть
msend %char% подкрасться
msend %char% заколоть
msend %char% подножка
msend %char% сбить
msend %char% оглушить
msend %char% метнуть 
msend %char% ярость
msend %char% уклониться
msend %char% парировать
msend %char% отравить
msend %char% найти
msend %char% подсмотреть
msend %char% лечить
msend %char% опознание
msend %char% пнуть
msend %char% спасти
msend %char% обезоружить
else
...
награда скиллами~
0 c 1
осторожный взломать украсть подкрасться заколоть подножка сбить оглушить метнуть ярость уклониться парировать отравить найти подсмотреть лечить опознание пнуть спасти обезоружить~
if %actor%==%quester%      
 if !%actor.skill(%cmd%)%             
   mskillturn %actor.name% %cmd% set
   %actor.setquest(926)%
   detach 92627 %self.id%  
 elseif %actor.skill(%cmd%)% < 151
   mskilladd %actor.name% %cmd% 50 
   %actor.setquest(926)%
   detach 92627 %self.id%
 else
 say Ты достиг такого мастерства, что я ничему не могу научить тебя большему.
 end    
